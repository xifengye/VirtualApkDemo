...
static int addattr32(struct nlmsghdr *n, size_t maxlen, int type, __u32 data)
{
	...
	if (NLMSG_ALIGN(n->nlmsg_len) + len > maxlen) {
		LOGE("addattr32: Error! max allowed bound %zu exceeded\n",maxlen);
		return -1;
	}
		     ...
		     const void *data, int alen)
{
	...
	if (NLMSG_ALIGN(n->nlmsg_len) + RTA_ALIGN(len) > maxlen) {
		LOGE("addattr_l ERROR: message exceeded bound of %zu\n",maxlen);
		return -1;
	}
		...
		for (h = (struct nlmsghdr *)buf; (size_t) status >= sizeof(*h);) {
			...
			if (l < 0 || len > status) {
				if (msg.msg_flags & MSG_TRUNC) {
					LOGE("Truncated message\n");
					return -1;
				}
				LOGE("!!!malformed message: len=%d\n", len);
				return -1;
			}
			...
			if (h->nlmsg_type == NLMSG_ERROR) {
				...
				if ((size_t) l < sizeof(struct nlmsgerr)) {
					LOGE( "ERROR truncated\n");
				} else {
					errno = -err->error;
					if (errno != -1)
						return 0;

					LOGE("RTNETLINK answers");
				}
				return -1;
			}
...
static int open_nl_sock()
{
	...
	if (fd < 0) {
		LOGE("Cannot open netlink socket");
		return -1;
	}
		...
		for (nl_msg = (struct nlmsghdr *)nlbuf;
			...
			case GET_CLOCK:
				if (can_attr[IFLA_CAN_CLOCK]) {
					...
					LOGE("no clock parameter data found\n");
			...
			case GET_BITTIMING_CONST:
				if (can_attr[IFLA_CAN_BITTIMING_CONST]) {
					...
					LOGE("no bittiming_const data found\n");
			...
			case GET_BERR_COUNTER:
				if (can_attr[IFLA_CAN_BERR_COUNTER]) {
					...
					LOGE( "no berr_counter data found\n");

				break;

			default:
				LOGE( "unknown acquire mode\n");
			}
		}
	}
...
int can_do_restart(const char *name)
{
	...
	if ((can_get_state(name, &state)) < 0) {
		LOGE( "cannot get bustate, ""something is seriously wrong\n");
		goto err_out;
	} else if (state != CAN_STATE_BUS_OFF) {
		LOGE("Device is not in BUS_OFF," " no use to restart\n");
		goto err_out;
	}

	if ((can_get_restart_ms(name, &restart_ms)) < 0) {
		LOGE( "cannot get restart_ms, " "something is seriously wrong\n");
		goto err_out;
